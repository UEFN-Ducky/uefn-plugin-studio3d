...
```
studio3d_tencent_rapid(prompt="a wooden chair with armrests", enable_pbr=true, wait=true)
...
```
studio3d_tripo(prompt="a fantasy crate", version="v3.1", enable_pbr=true)
...
```
studio3d_trellis(image="C:/path/to/product.png", enable_pbr=true, wait=true)
```
...
studio3d_trellis(image="<downloaded image path>", enable_pbr=true, wait=true)
...
```
studio3d_tencent_rapid(prompt="a ceramic vase", enable_pbr=true, wait=true)
# use result asset URL:
studio3d_repair(model_url="https://storage.3daistudio.com/assets/model.glb", wait=true)
studio3d_convert(model_url="<repaired asset URL>", output_format="stl", wait=true)
```

## 7. Remove background before 3D

```
studio3d_remove_bg(image="C:/path/to/product.jpg", wait=true)
studio3d_trellis(image="<cleaned png path>", enable_pbr=true, wait=true)
```
